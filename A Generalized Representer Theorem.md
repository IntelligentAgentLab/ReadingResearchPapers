...
$$
f^*\in
\operatorname{span}\{k(\cdot,x_1),\ldots,k(\cdot,x_m)\}
$$
...
c\bigl((x_1,y_1,(K\alpha)_1),\ldots,(x_m,y_m,(K\alpha)_m)\bigr)
...
\boxed{
\tilde f(\cdot)
=\sum_{i=1}^{m}\alpha_i k(x_i,\cdot)
+\sum_{p=1}^{M}\beta_p\psi_p(\cdot)
}
$$
...
$$
k_\theta(x,z)
=k_0\bigl(\phi_\theta(x),\phi_\theta(z)\bigr)
$$
